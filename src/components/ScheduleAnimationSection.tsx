        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="relative">
            <div ref={animationRef} className="glass-card p-8 min-h-[500px] relative overflow-hidden">
              ...
              {activeStep === 0 && <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      {["Иванов И.И.", "Петрова А.С.", "Сидоров В.П.", "Козлова М.Н.", "Федоров Д.А."].map((teacher, i) => <div key={i} className="p-2 bg-glass-secondary border border-glass-border rounded-xl text-xs animate-fade-in" style={{
                    ...
                    <div className="space-y-2">
                      ...
                      {subjects.map((subject, i) => <div key={i} className="p-2 bg-glass-secondary border border-glass-border rounded-xl text-xs animate-fade-in" style={{
              ...
              {/* Stage 1: Analysis */}
              {activeStep === 1 && <div className="flex flex-col items-center justify-center h-full space-y-6">
                  <div className="relative">
                  ...
                  <div className="absolute inset-0 pointer-events-none">
                    {[...Array(12)].map((_, i) => <div key={i} className="absolute w-2 h-2 bg-stellar-accent rounded-full opacity-60 animate-schedule-flow" style={{
              ...
              {activeStep === 2 && <div className="space-y-4">
                  ...
                  teacher: "Федоров Д.А.",
                  conflict: false
                }].map((item, i) => <div key={i} className={`p-3 rounded-xl text-xs transition-all duration-500 ${item.conflict ? 'bg-destructive/20 border border-destructive/50 text-destructive animate-pulse' : 'bg-stellar-primary/20 border border-stellar-accent/50 text-stellar-accent'}`}>
              ...
              {activeStep === 3 && <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {timeSlots.map((time, timeIndex) => <div key={timeIndex} className="space-y-2">
                        <div className="text-xs text-stellar-accent font-medium text-center p-2 glass-card glow-effect">
                          {time}
                        </div>
                        {subjects.slice(0, 3).map((subject, subjectIndex) => <div key={`${timeIndex}-${subjectIndex}`} className="p-2 rounded-xl text-xs text-center bg-stellar-primary/30 border border-stellar-accent text-stellar-accent transform animate-scale-in glow-effect" style={{
          ...
          <div className="space-y-8">
            <div className="min-h-[0px] flex items-center justify-center relative">
                ...
                return <div key={index} className={`glass-card p-8 transition-all duration-700 ease-in-out ${isActive ? 'opacity-100 scale-100 ring-2 ring-stellar-accent/50 glow-effect' : 'opacity-0 scale-95 pointer-events-none absolute inset-0'}`}>
                      ...
                      <div className="flex flex-col items-center text-center space-y-6 relative z-10">
                        ...
                        <div className="flex space-x-2">
                          {scheduleSteps.map((_, stepIndex) => <div key={stepIndex} className={`w-2 h-2 rounded-full transition-all duration-300 ${stepIndex === activeStep ? 'bg-stellar-accent scale-125' : stepIndex < activeStep ? 'bg-stellar-accent/60' : 'bg-glass-border'}`} />)}
                        </div>
                      </div>
                    </div>;
              })}
